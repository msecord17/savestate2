...
const LEGACY_TO_TIMELINE: Record<string, string> = {
...
export function normalizeEraKeyToTimeline(input?: string | null): TimelineEraKey {
  const k = (input ?? "").trim();
  if (!k) return "unknown";
  if (k in ORIGIN_BUCKET_META) return k as TimelineEraKey;
  return (LEGACY_TO_TIMELINE[k] ?? "unknown") as TimelineEraKey;
}

export function normalizeTopEraForProfile(inputKey?: string | null) {
  const raw = (inputKey ?? "").trim();
  const timeline_key = normalizeEraKeyToTimeline(raw);
  const meta = ORIGIN_BUCKET_META[timeline_key] ?? ORIGIN_BUCKET_META.unknown;

  const legacy_key = raw && !(raw in ORIGIN_BUCKET_META) ? raw : null;
